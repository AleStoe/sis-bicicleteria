...
def test_rechaza_pago_directo_a_venta_entregada_con_deuda(client, db_conn, seed_venta_basica):
    ...
    assert venta_antes["estado"] == "entregada"
    assert Decimal(str(venta_antes["saldo_pendiente"])) > Decimal("0.00")
    assert len(pagos_antes) == 1
...
def test_rechaza_reversion_pago_sin_permiso(client, db_conn, seed_venta_basica):
    ...
    venta_antes = get_venta(db_conn, venta_id)
    movimientos_caja_antes = get_caja_movimientos(db_conn, caja_id)
    auditoria_antes = get_auditoria_by_entidad(db_conn, "pago", pago_id)
    ...
    venta_despues = get_venta(db_conn, venta_id)
    movimientos_caja_despues = get_caja_movimientos(db_conn, caja_id)
    auditoria_despues = get_auditoria_by_entidad(db_conn, "pago", pago_id)
...
def test_no_permite_revertir_pago_de_venta_entregada(client, db_conn, seed_venta_basica):
    venta_id = crear_venta_base(client, seed_venta_basica)
