...
CREATE TABLE public.agenda_taller (
    ...
    CONSTRAINT chk_agenda_taller_tipo_turno CHECK (((tipo_turno)::text = ANY ((ARRAY['reparacion_comun'::character varying, 'service_postventa_30_dias'::character varying, 'garantia'::character varying, 'consulta_revision'::character varying])::text[])))
);
...
CREATE TABLE public.caja_movimientos (
    ...
    CONSTRAINT chk_caja_mov_submedio CHECK (((submedio)::text = ANY (ARRAY[('efectivo'::character varying)::text, ('transferencia'::character varying)::text, ('mercadopago'::character varying)::text, ('tarjeta'::character varying)::text]))),
    CONSTRAINT chk_caja_mov_tipo CHECK (((tipo_movimiento)::text = ANY (ARRAY[('ingreso'::character varying)::text, ('egreso'::character varying)::text, ('ajuste'::character varying)::text]))),
    CONSTRAINT chk_caja_movimientos_direccion_ajuste CHECK (((direccion_ajuste IS NULL) OR ((direccion_ajuste)::text = ANY (ARRAY[('positivo'::character varying)::text, ('negativo'::character varying)::text]))))
);
...
CREATE TABLE public.cajas (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT chk_cajas_estado CHECK (((estado)::text = ANY (ARRAY[('abierta'::character varying)::text, ('cerrada'::character varying)::text])))
);
...
CREATE TABLE public.capital_movimientos (
    ...
    CONSTRAINT chk_capital_movimientos_estado CHECK (((estado)::text = ANY (ARRAY[('activo'::character varying)::text, ('anulado'::character varying)::text]))),
    CONSTRAINT chk_capital_movimientos_medio_pago CHECK (((medio_pago IS NULL) OR ((medio_pago)::text = ANY (ARRAY[('efectivo'::character varying)::text, ('transferencia'::character varying)::text, ('mercadopago'::character varying)::text, ('tarjeta'::character varying)::text])))),
    CONSTRAINT chk_capital_movimientos_monto CHECK ((monto > (0)::numeric)),
    CONSTRAINT chk_capital_movimientos_tipo CHECK (((tipo_movimiento)::text = ANY (ARRAY[('aporte_capital'::character varying)::text, ('prestamo_socio'::character varying)::text, ('devolucion_prestamo'::character varying)::text, ('retiro_personal'::character varying)::text, ('distribucion_ganancia'::character varying)::text])))
);
...
CREATE TABLE public.capital_participantes (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT chk_capital_participantes_tipo CHECK (((tipo)::text = ANY (ARRAY[('persona'::character varying)::text, ('fondo'::character varying)::text])))
);
...
CREATE TABLE public.cierres_rentabilidad (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    financiacion_cobrada numeric(14,2) DEFAULT 0 NOT NULL,
    costos_financieros numeric(14,2) DEFAULT 0 NOT NULL,
    resultado_financiero numeric(14,2) DEFAULT 0 NOT NULL,
    margen_real numeric(14,2) DEFAULT 0 NOT NULL,
    CONSTRAINT chk_cierres_rentabilidad_estado CHECK (((estado)::text = ANY (ARRAY[('cerrado'::character varying)::text, ('anulado'::character varying)::text]))),
...
CREATE TABLE public.clientes (
    ...
    CONSTRAINT chk_clientes_tipo_cliente CHECK (((tipo_cliente)::text = ANY (ARRAY[('minorista'::character varying)::text, ('mayorista'::character varying)::text, ('consumidor_final'::character varying)::text])))
);
...
CREATE TABLE public.configuracion_negocio (
    ...
    nombre_negocio text DEFAULT 'Emprendimiento Agus'::text NOT NULL,
    direccion text,
    telefono text,
    horarios_retiro text DEFAULT 'Lunes a viernes
09:00 a 12:00
16:30 a 20:00

Sábados
09:00 a 12:00
17:00 a 19:00'::text NOT NULL,
    whatsapp_cierre text DEFAULT 'Gracias por confiar en nosotros.
¡Te esperamos!'::text NOT NULL,
    texto_beneficio_pago text DEFAULT 'Abonando en efectivo o transferencia tenés 10% de descuento sobre los trabajos presupuestados.'::text NOT NULL,
    whatsapp_retiro_mostrar_total boolean DEFAULT true NOT NULL,
    whatsapp_retiro_mostrar_trabajos boolean DEFAULT true NOT NULL,
    plantilla_turno_confirmacion text DEFAULT 'Hola {cliente_nombre} 👋
...
{fecha_prometida_bloque}{whatsapp_cierre} 🚲'::text NOT NULL,
    plantilla_turno_recordatorio text DEFAULT 'Hola {cliente_nombre} 👋
...
{whatsapp_cierre} 🚲'::text NOT NULL,
    plantilla_turno_aviso text DEFAULT 'Hola {cliente_nombre} 👋
...
{fecha_prometida_bloque}Cualquier cambio te avisamos por este medio. 🚲'::text NOT NULL,
    plantilla_retiro_taller text DEFAULT '🚲 *¡Tu bicicleta está lista para retirar!*
...
🙌 {whatsapp_cierre} 🚲'::text NOT NULL,
    plantilla_cotizacion_whatsapp text DEFAULT 'Hola {cliente_nombre}, te paso la cotización {numero_cotizacion}.

Tipo: {tipo_cotizacion}.
{consulta_bloque}{detalle_bloque}
Total estimado: {total}
{validez_bloque}
No reserva stock ni genera deuda hasta que la confirmes.'::text NOT NULL,
    condiciones_presupuesto_taller text DEFAULT 'Presupuesto no fiscal. No válido como factura.
...
{texto_beneficio_pago}'::text NOT NULL,
    condiciones_cotizacion text DEFAULT 'Cotización no fiscal. No válida como factura.
No reserva stock ni genera deuda hasta que sea confirmada.
Los precios pueden variar al vencer la validez indicada.'::text NOT NULL,
    actualizado_en timestamp with time zone DEFAULT now() NOT NULL,
    porcentaje_descuento_contado_calculadora_precios numeric(6,3) DEFAULT 10.000 CONSTRAINT configuracion_negocio_porcentaje_descuento_contado_cal_not_null NOT NULL,
...
CREATE TABLE public.cotizacion_items (
    ...
    CONSTRAINT chk_cotizacion_items_tipo CHECK (((tipo_item)::text = ANY (ARRAY[('producto'::character varying)::text, ('servicio_taller'::character varying)::text, ('linea_libre'::character varying)::text])))
);
...
CREATE TABLE public.cotizaciones (
    ...
    CONSTRAINT chk_cotizaciones_estado CHECK (((estado)::text = ANY (ARRAY[('borrador'::character varying)::text, ('enviada'::character varying)::text, ('aceptada'::character varying)::text, ('convertida'::character varying)::text, ('rechazada'::character varying)::text, ('vencida'::character varying)::text, ('cancelada'::character varying)::text]))),
    ...
    CONSTRAINT chk_cotizaciones_tipo CHECK (((tipo)::text = ANY (ARRAY[('venta'::character varying)::text, ('reparacion'::character varying)::text]))),
    CONSTRAINT chk_cotizaciones_tipo_precio CHECK (((tipo_precio)::text = ANY (ARRAY[('minorista'::character varying)::text, ('mayorista'::character varying)::text])))
);
...
CREATE TABLE public.credito_movimientos (
    ...
    CONSTRAINT chk_credito_movimientos_tipo CHECK (((tipo_movimiento)::text = ANY (ARRAY[('credito_generado'::character varying)::text, ('aplicacion_a_venta'::character varying)::text, ('reintegro'::character varying)::text, ('ajuste'::character varying)::text, ('anulacion_credito'::character varying)::text])))
);
...
CREATE TABLE public.creditos_cliente (
    ...
    observacion text,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT chk_creditos_cliente_estado CHECK (((estado)::text = ANY (ARRAY[('abierto'::character varying)::text, ('aplicado_parcial'::character varying)::text, ('aplicado_total'::character varying)::text, ('anulado'::character varying)::text]))),
    CONSTRAINT chk_creditos_cliente_origen_tipo CHECK (((origen_tipo)::text = ANY (ARRAY[('venta'::character varying)::text, ('reserva'::character varying)::text, ('orden_taller'::character varying)::text, ('ajuste_manual'::character varying)::text]))),
...
CREATE TABLE public.deuda_movimientos (
    ...
    CONSTRAINT chk_deuda_mov_tipo CHECK (((tipo_movimiento)::text = ANY (ARRAY[('cargo'::character varying)::text, ('pago'::character varying)::text, ('recargo'::character varying)::text, ('ajuste'::character varying)::text, ('reversion'::character varying)::text])))
);
...
CREATE TABLE public.deudas_cliente (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT chk_deuda_origen_tipo CHECK (((origen_tipo)::text = ANY (ARRAY[('venta'::character varying)::text, ('orden_taller'::character varying)::text, ('financiacion'::character varying)::text]))),
    ...
    CONSTRAINT chk_deudas_estado CHECK (((estado)::text = ANY (ARRAY[('abierta'::character varying)::text, ('cerrada'::character varying)::text, ('anulada'::character varying)::text]))),
...
CREATE TABLE public.gastos_movimientos (
    ...
    id_usuario bigint NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT chk_gastos_movimientos_tipo CHECK (((tipo_movimiento)::text = ANY (ARRAY[('creacion'::character varying)::text, ('correccion'::character varying)::text, ('anulacion'::character varying)::text, ('pago'::character varying)::text, ('reversion_pago'::character varying)::text]))),
...
CREATE TABLE public.gastos_operativos (
    ...
    CONSTRAINT chk_gastos_operativos_estado CHECK (((estado)::text = ANY (ARRAY[('activo'::character varying)::text, ('anulado'::character varying)::text]))),
...
--
-- Name: inventario_fisico_items; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.inventario_fisico_items (
    id bigint NOT NULL,
    id_inventario bigint NOT NULL,
    id_variante bigint NOT NULL,
    stock_sistema numeric(14,3) DEFAULT 0 NOT NULL,
    stock_contado numeric(14,3),
    diferencia numeric(14,3) DEFAULT 0 NOT NULL,
    nota text,
    contado_at timestamp with time zone,
    id_usuario_conteo bigint,
    movimiento_stock_id bigint,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT chk_inventario_fisico_stock_no_negativo CHECK (((stock_sistema >= (0)::numeric) AND ((stock_contado IS NULL) OR (stock_contado >= (0)::numeric))))
);


--
-- Name: inventario_fisico_items_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

CREATE SEQUENCE public.inventario_fisico_items_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
--
-- Name: inventario_fisico_items_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--

ALTER SEQUENCE public.inventario_fisico_items_id_seq OWNED BY public.inventario_fisico_items.id;


--
-- Name: inventarios_fisicos; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE public.inventarios_fisicos (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT chk_inventarios_fisicos_estado CHECK (((estado)::text = ANY (ARRAY[('abierto'::character varying)::text, ('cerrado'::character varying)::text, ('cancelado'::character varying)::text])))
);


--
-- Name: inventarios_fisicos_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

CREATE SEQUENCE public.inventarios_fisicos_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
--
-- Name: inventarios_fisicos_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--

ALTER SEQUENCE public.inventarios_fisicos_id_seq OWNED BY public.inventarios_fisicos.id;
...
CREATE TABLE public.ofertas (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT chk_ofertas_precios_positivos CHECK (((precio_regular_referencia > (0)::numeric) AND (precio_oferta > (0)::numeric) AND (precio_oferta < precio_regular_referencia))),
...
CREATE TABLE public.ordenes_taller (
    ...
    CONSTRAINT chk_ordenes_taller_estado CHECK (((estado)::text = ANY (ARRAY[('ingresada'::character varying)::text, ('presupuestada'::character varying)::text, ('esperando_aprobacion'::character varying)::text, ('esperando_repuestos'::character varying)::text, ('en_reparacion'::character varying)::text, ('terminada'::character varying)::text, ('facturada'::character varying)::text, ('lista_para_retirar'::character varying)::text, ('retirada'::character varying)::text, ('cancelada'::character varying)::text]))),
...
CREATE TABLE public.ordenes_taller_items (
    ...
    CONSTRAINT chk_orden_taller_item_cobertura CHECK (((valor_cobertura_unitario >= (0)::numeric) AND (valor_cobertura_unitario <= precio_unitario) AND ((valor_cobertura_unitario = (0)::numeric) OR (((tipo_item)::text = ANY ((ARRAY['repuesto'::character varying, 'servicio'::character varying])::text[])) AND (NULLIF(TRIM(BOTH FROM motivo_cobertura), ''::text) IS NOT NULL))))),
    ...
    CONSTRAINT chk_oti_tipo_item CHECK (((tipo_item)::text = ANY (ARRAY[('repuesto'::character varying)::text, ('servicio'::character varying)::text])))
);
...
--
-- Name: ordenes_taller_notas; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE public.ordenes_taller_notas (
    ...
    CONSTRAINT chk_ordenes_taller_notas_estado CHECK (((estado)::text = ANY ((ARRAY['activa'::character varying, 'resuelta'::character varying, 'archivada'::character varying])::text[]))),
    CONSTRAINT chk_ordenes_taller_notas_tipo CHECK (((tipo)::text = ANY ((ARRAY['interna'::character varying, 'cliente'::character varying, 'recomendacion_futura'::character varying, 'alerta_tecnica'::character varying])::text[])))
);


--
-- Name: ordenes_taller_notas_id_seq; Type: SEQUENCE; Schema: public; Owner: -
--

CREATE SEQUENCE public.ordenes_taller_notas_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
--
-- Name: ordenes_taller_notas_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: -
--

ALTER SEQUENCE public.ordenes_taller_notas_id_seq OWNED BY public.ordenes_taller_notas.id;
...
CREATE TABLE public.pagos (
    ...
    id_plan_financiero bigint,
    porcentaje_costo_financiero_aplicado numeric(10,4) DEFAULT 0 NOT NULL,
    monto_costo_financiero numeric(14,2) DEFAULT 0 NOT NULL,
    monto_neto_liquidado numeric(14,2) DEFAULT 0 NOT NULL,
    CONSTRAINT chk_pagos_costo_financiero_no_negativo CHECK (((porcentaje_costo_financiero_aplicado >= (0)::numeric) AND (monto_costo_financiero >= (0)::numeric) AND (monto_neto_liquidado >= (0)::numeric))),
    CONSTRAINT chk_pagos_estado CHECK (((estado)::text = ANY (ARRAY[('confirmado'::character varying)::text, ('revertido'::character varying)::text, ('devuelto_externo'::character varying)::text]))),
    CONSTRAINT chk_pagos_medio_pago CHECK (((medio_pago)::text = ANY (ARRAY[('efectivo'::character varying)::text, ('transferencia'::character varying)::text, ('mercadopago'::character varying)::text, ('tarjeta'::character varying)::text]))),
    CONSTRAINT chk_pagos_monto_positivo CHECK ((monto_total_cobrado > (0)::numeric)),
    CONSTRAINT chk_pagos_neto_no_supera_bruto CHECK ((monto_neto_liquidado <= monto_total_cobrado)),
    CONSTRAINT chk_pagos_origen_tipo CHECK (((origen_tipo)::text = ANY (ARRAY[('venta'::character varying)::text, ('reserva'::character varying)::text, ('orden_taller'::character varying)::text, ('deuda_cliente'::character varying)::text])))
);
...
CREATE TABLE public.precios_movimientos (
    ...
    CONSTRAINT chk_precios_mov_tipo CHECK (((tipo_movimiento)::text = ANY (ARRAY[('actualizacion_manual'::character varying)::text, ('actualizacion_por_ingreso_stock'::character varying)::text, ('actualizacion_por_lista_proveedor'::character varying)::text, ('correccion_error'::character varying)::text, ('cambio_margen'::character varying)::text])))
);
...
CREATE TABLE public.productos (
    ...
    rubro character varying(30) DEFAULT 'REPUESTOS'::character varying,
    CONSTRAINT chk_productos_serializable_stockeable CHECK (((serializable = false) OR ((serializable = true) AND (stockeable = true)))),
    CONSTRAINT chk_productos_tipo_item CHECK (((tipo_item)::text = ANY (ARRAY[('producto'::character varying)::text, ('servicio'::character varying)::text])))
);
...
CREATE TABLE public.reglas_comerciales (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT chk_reglas_comerciales_medio_pago CHECK (((medio_pago IS NULL) OR ((medio_pago)::text = ANY (ARRAY[('efectivo'::character varying)::text, ('transferencia'::character varying)::text, ('mercadopago'::character varying)::text, ('tarjeta'::character varying)::text])))),
    ...
    CONSTRAINT chk_reglas_comerciales_tipo CHECK (((tipo)::text = ANY (ARRAY[('descuento'::character varying)::text, ('recargo'::character varying)::text]))),
...
CREATE TABLE public.reglas_precio (
    ...
    CONSTRAINT chk_reglas_precio_tipo_cliente CHECK (((tipo_cliente)::text = ANY (ARRAY[('minorista'::character varying)::text, ('mayorista'::character varying)::text])))
);
...
CREATE TABLE public.reserva_eventos (
    ...
    id_usuario bigint NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT chk_reserva_eventos_tipo CHECK (((tipo_evento)::text = ANY (ARRAY[('creada'::character varying)::text, ('senal_recibida'::character varying)::text, ('vencida'::character varying)::text, ('cancelada'::character varying)::text, ('sena_perdida'::character varying)::text, ('convertida_en_venta'::character varying)::text, ('asignacion_serie'::character varying)::text, ('cambio_estado'::character varying)::text])))
);
...
CREATE TABLE public.reservas (
    ...
    CONSTRAINT chk_reservas_estado CHECK (((estado)::text = ANY (ARRAY[('activa'::character varying)::text, ('vencida'::character varying)::text, ('cancelada'::character varying)::text, ('convertida_en_venta'::character varying)::text]))),
    ...
    CONSTRAINT chk_reservas_tipo CHECK (((tipo_reserva)::text = ANY (ARRAY[('comun'::character varying)::text, ('financiada'::character varying)::text])))
);
...
CREATE TABLE public.venta_items (
    ...
    id_oferta bigint,
    precio_catalogo_original numeric(14,2),
    descuento_oferta_unitario numeric(14,2) DEFAULT 0 NOT NULL,
    oferta_nombre_snapshot character varying(120),
    ...
    CONSTRAINT chk_venta_items_descuento_oferta CHECK (((descuento_oferta_unitario >= (0)::numeric) AND ((id_oferta IS NULL) OR ((precio_catalogo_original IS NOT NULL) AND (precio_catalogo_original > precio_lista) AND (descuento_oferta_unitario = (precio_catalogo_original - precio_lista)) AND (NULLIF(TRIM(BOTH FROM oferta_nombre_snapshot), ''::text) IS NOT NULL))))),
    ...
    CONSTRAINT chk_venta_items_tipo_item CHECK (((tipo_item)::text = ANY (ARRAY[('producto'::character varying)::text, ('servicio_taller'::character varying)::text])))
);
...
CREATE TABLE public.ventas (
    ...
    CONSTRAINT chk_ventas_estado CHECK (((estado)::text = ANY (ARRAY[('creada'::character varying)::text, ('pagada_parcial'::character varying)::text, ('pagada_total'::character varying)::text, ('entregada'::character varying)::text, ('anulada'::character varying)::text, ('devuelta'::character varying)::text, ('devuelta_parcial'::character varying)::text]))),
...
CREATE TABLE public.venta_item_devoluciones (
    ...
    CONSTRAINT chk_vid_tipo_item CHECK (((tipo_item)::text = ANY (ARRAY[('producto'::character varying)::text, ('servicio_taller'::character varying)::text]))),
...
CREATE TABLE public.venta_reglas_aplicadas (
    ...
    CONSTRAINT chk_venta_reglas_aplicadas_tipo CHECK (((tipo)::text = ANY (ARRAY[('descuento'::character varying)::text, ('recargo'::character varying)::text])))
);
...
ALTER TABLE ONLY public.cotizaciones ALTER COLUMN numero SET DEFAULT ('COT-'::text || lpad((nextval('public.cotizaciones_numero_seq'::regclass))::text, 6, '0'::text));
...
--
-- Name: inventario_fisico_items id; Type: DEFAULT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.inventario_fisico_items ALTER COLUMN id SET DEFAULT nextval('public.inventario_fisico_items_id_seq'::regclass);


--
-- Name: inventarios_fisicos id; Type: DEFAULT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.inventarios_fisicos ALTER COLUMN id SET DEFAULT nextval('public.inventarios_fisicos_id_seq'::regclass);
...
--
-- Name: ordenes_taller_notas id; Type: DEFAULT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.ordenes_taller_notas ALTER COLUMN id SET DEFAULT nextval('public.ordenes_taller_notas_id_seq'::regclass);
...
--
-- Name: inventario_fisico_items inventario_fisico_items_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.inventario_fisico_items
    ADD CONSTRAINT inventario_fisico_items_pkey PRIMARY KEY (id);


--
-- Name: inventarios_fisicos inventarios_fisicos_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.inventarios_fisicos
    ADD CONSTRAINT inventarios_fisicos_pkey PRIMARY KEY (id);
...
--
-- Name: ordenes_taller_notas ordenes_taller_notas_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.ordenes_taller_notas
    ADD CONSTRAINT ordenes_taller_notas_pkey PRIMARY KEY (id);
...
--
-- Name: inventario_fisico_items uq_inventario_fisico_item; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.inventario_fisico_items
    ADD CONSTRAINT uq_inventario_fisico_item UNIQUE (id_inventario, id_variante);
...
--
-- Name: idx_agenda_taller_bicicleta_tipo; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_agenda_taller_venta_origen; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_cotizacion_items_cotizacion; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_cotizacion_items_cotizacion ON public.cotizacion_items USING btree (id_cotizacion, orden, id);


--
-- Name: idx_cotizaciones_bicicleta_cliente; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_cotizaciones_bicicleta_cliente ON public.cotizaciones USING btree (id_bicicleta_cliente);
...
--
-- Name: idx_cotizaciones_tipo_estado; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_cotizaciones_tipo_estado ON public.cotizaciones USING btree (tipo, estado);
...
--
-- Name: idx_inventario_fisico_items_inventario; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_inventario_fisico_items_inventario ON public.inventario_fisico_items USING btree (id_inventario);


--
-- Name: idx_inventarios_fisicos_sucursal_estado; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_inventarios_fisicos_sucursal_estado ON public.inventarios_fisicos USING btree (id_sucursal, estado);
...
--
-- Name: idx_ofertas_variante_vigencia; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_ofertas_variante_vigencia ON public.ofertas USING btree (id_variante, activa, fecha_desde, fecha_hasta);
...
--
-- Name: idx_ordenes_taller_notas_bicicleta; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_ordenes_taller_notas_orden; Type: INDEX; Schema: public; Owner: -
--
...
--
-- Name: idx_pagos_plan_financiero; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_pagos_plan_financiero ON public.pagos USING btree (id_plan_financiero);
...
--
-- Name: agenda_taller agenda_taller_id_venta_origen_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: cotizacion_items cotizacion_items_id_cotizacion_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: cotizacion_items cotizacion_items_id_servicio_taller_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: cotizacion_items cotizacion_items_id_variante_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: cotizaciones cotizaciones_id_bicicleta_cliente_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: cotizaciones cotizaciones_id_orden_taller_convertida_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: cotizaciones cotizaciones_id_sucursal_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: cotizaciones cotizaciones_id_usuario_actualiza_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: cotizaciones cotizaciones_id_usuario_creador_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: cotizaciones cotizaciones_id_venta_convertida_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: ordenes_taller_notas fk_ordenes_taller_notas_bicicleta; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: ordenes_taller_notas fk_ordenes_taller_notas_orden; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: ordenes_taller_notas fk_ordenes_taller_notas_usuario_actualiza; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: ordenes_taller_notas fk_ordenes_taller_notas_usuario_creador; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: inventario_fisico_items inventario_fisico_items_id_inventario_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.inventario_fisico_items
    ADD CONSTRAINT inventario_fisico_items_id_inventario_fkey FOREIGN KEY (id_inventario) REFERENCES public.inventarios_fisicos(id) ON DELETE CASCADE;


--
-- Name: inventario_fisico_items inventario_fisico_items_id_usuario_conteo_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.inventario_fisico_items
    ADD CONSTRAINT inventario_fisico_items_id_usuario_conteo_fkey FOREIGN KEY (id_usuario_conteo) REFERENCES public.usuarios(id);


--
-- Name: inventario_fisico_items inventario_fisico_items_id_variante_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.inventario_fisico_items
    ADD CONSTRAINT inventario_fisico_items_id_variante_fkey FOREIGN KEY (id_variante) REFERENCES public.variantes(id);


--
-- Name: inventario_fisico_items inventario_fisico_items_movimiento_stock_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.inventario_fisico_items
    ADD CONSTRAINT inventario_fisico_items_movimiento_stock_id_fkey FOREIGN KEY (movimiento_stock_id) REFERENCES public.movimientos_stock(id);


--
-- Name: inventarios_fisicos inventarios_fisicos_id_sucursal_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.inventarios_fisicos
    ADD CONSTRAINT inventarios_fisicos_id_sucursal_fkey FOREIGN KEY (id_sucursal) REFERENCES public.sucursales(id);


--
-- Name: inventarios_fisicos inventarios_fisicos_id_usuario_cierre_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.inventarios_fisicos
    ADD CONSTRAINT inventarios_fisicos_id_usuario_cierre_fkey FOREIGN KEY (id_usuario_cierre) REFERENCES public.usuarios(id);


--
-- Name: inventarios_fisicos inventarios_fisicos_id_usuario_creador_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.inventarios_fisicos
    ADD CONSTRAINT inventarios_fisicos_id_usuario_creador_fkey FOREIGN KEY (id_usuario_creador) REFERENCES public.usuarios(id);
...
--
-- Name: ofertas ofertas_id_usuario_actualizador_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.ofertas
    ADD CONSTRAINT ofertas_id_usuario_actualizador_fkey FOREIGN KEY (id_usuario_actualizador) REFERENCES public.usuarios(id);


--
-- Name: ofertas ofertas_id_usuario_creador_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.ofertas
    ADD CONSTRAINT ofertas_id_usuario_creador_fkey FOREIGN KEY (id_usuario_creador) REFERENCES public.usuarios(id);


--
-- Name: ofertas ofertas_id_variante_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.ofertas
    ADD CONSTRAINT ofertas_id_variante_fkey FOREIGN KEY (id_variante) REFERENCES public.variantes(id);
...
--
-- Name: pagos pagos_id_plan_financiero_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.pagos
    ADD CONSTRAINT pagos_id_plan_financiero_fkey FOREIGN KEY (id_plan_financiero) REFERENCES public.tarjeta_planes(id);
...
--
-- Name: venta_items venta_items_id_oferta_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.venta_items
    ADD CONSTRAINT venta_items_id_oferta_fkey FOREIGN KEY (id_oferta) REFERENCES public.ofertas(id);
